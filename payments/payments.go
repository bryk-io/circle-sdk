...
// CapturePayment the given amount will be captured for the authorized payment if
// possible. If no amount is specified, the full amount will be captured. You can
// only capture once per authorization. A successful response does not mean the
// payment has been captured. It only means the capture request was successfully
// submitted.
...
// will not be charged & the payment will never settle. Otherwise, the payment will
// be refunded meaning the payment source will be charged & the payment will be
// refunded from deductions of future settlements. Not all payments are eligible
...
// RefundPayment the payment source will be refunded if possible. Not all payments
// are eligible to be canceled. A successful response does not mean the payment has
// been refunded; it only means the refund request is successfully submitted.
